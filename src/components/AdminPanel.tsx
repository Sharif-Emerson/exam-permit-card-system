      ...
      <div className="flex min-w-0 flex-1 flex-col overflow-hidden">
        ...
        <header className="flex h-16 flex-shrink-0 items-center gap-4 border-b border-gray-200 bg-white px-4 pt-[max(env(safe-area-inset-top),0.5rem)] dark:border-slate-800 dark:bg-[radial-gradient(circle_at_top_left,_rgba(30,58,138,0.45),_transparent_30%),radial-gradient(circle_at_bottom_right,_rgba(127,29,29,0.35),_transparent_28%),linear-gradient(180deg,_#020617_0%,_#052e16_55%,_#1f2937_100%)] sm:px-6 lg:pt-0">
          ...
          <div className="ml-auto flex items-center gap-3">
            ...
            <div className="hidden items-center gap-2 sm:flex">
              <div className="flex h-8 w-8 items-center justify-center rounded-full bg-emerald-100 dark:bg-emerald-900/40 text-sm font-bold text-emerald-700 dark:text-emerald-300">
                {user?.name?.[0]?.toUpperCase() ?? 'A'}
              </div>
              <div>
        ...
        {(successMessage || createdStudentWelcome) && (
          <div className="flex-shrink-0 px-6 pt-4">
            {successMessage && (
              <div className="mb-2 flex items-start gap-3 rounded-lg border border-green-200 dark:border-green-700 bg-green-50 dark:bg-green-950/40 p-3 text-sm text-green-700 dark:text-green-300">
            ...
            {createdStudentWelcome && (
              <div className="mb-2 rounded-lg border border-emerald-200 dark:border-emerald-700 bg-emerald-50 dark:bg-emerald-950/40 p-4 text-sm text-emerald-900 dark:text-emerald-100">
                <div className="mb-3 flex items-start justify-between gap-3">
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-[0.25em] text-emerald-700 dark:text-emerald-300">Welcome Message</p>
                    <h2 className="mt-1 text-base font-semibold text-emerald-950 dark:text-white">New student account ready</h2>
                  </div>
                  <button
                    type="button"
                ...
                <div className="grid gap-2 sm:grid-cols-2">
                  <div className="rounded-md bg-white/70 dark:bg-slate-800/60 px-3 py-2">
                    <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-emerald-600 dark:text-emerald-400">Student Name</p>
                    <p className="mt-1 font-medium text-emerald-950 dark:text-white">{createdStudentWelcome.name}</p>
                  </div>
                  <div className="rounded-md bg-white/70 dark:bg-slate-800/60 px-3 py-2">
                    <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-emerald-600 dark:text-emerald-400">Registration Number</p>
                    <p className="mt-1 font-medium text-emerald-950 dark:text-white">{createdStudentWelcome.studentId}</p>
                  </div>
                  <div className="rounded-md bg-white/70 dark:bg-slate-800/60 px-3 py-2">
                    <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-emerald-600 dark:text-emerald-400">Email</p>
                    <p className="mt-1 font-medium text-emerald-950 dark:text-white">{createdStudentWelcome.email}</p>
                  </div>
                  <div className="rounded-md bg-white/70 dark:bg-slate-800/60 px-3 py-2">
                    ...
                    <p className="mt-1 font-medium text-emerald-950 dark:text-white">{createdStudentWelcome.password}</p>
                  </div>
                </div>
              </div>
            )}
                  ...
                  <div className="rounded-xl border border-red-200/80 bg-[linear-gradient(145deg,_rgba(255,241,242,0.94),_rgba(255,251,235,0.9))] shadow-sm shadow-red-200/40">
                    <div className="flex items-center justify-between border-b border-gray-100 px-5 py-4">
                      ...
                      <button
                        ...
                        className="rounded-lg border border-red-200 dark:border-red-700 bg-white/90 dark:bg-slate-800 px-3 py-1.5 text-xs font-medium text-red-700 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-950/40"
                      >
                        Review list
                      </button>
                    </div>
                      ...
                      {students.filter((student) => student.feesBalance > 0).slice(0, 5).map((student) => (
                        <div key={student.id} className="flex items-center justify-between gap-4 px-5 py-4">
                          ...
                          <div className="text-right">
                            <p className="text-sm font-semibold text-red-600 dark:text-red-400">{formatMoney(student.feesBalance, activeCurrencyCode)}</p>
                  ...
                  <div className="rounded-xl border border-amber-200/80 bg-[linear-gradient(145deg,_rgba(254,252,232,0.95),_rgba(255,247,237,0.92))] p-6 shadow-sm shadow-amber-200/45">
                    ...
                    <div className="mt-4 grid grid-cols-2 gap-3">
                      <div className="rounded-xl bg-emerald-50 dark:bg-emerald-950/40 p-4 text-center">
                        <p className="text-2xl font-bold text-emerald-700 dark:text-emerald-300">{permitStatusCounts.issued}</p>
                        <p className="mt-1 text-xs text-emerald-500 dark:text-emerald-400">Issued</p>
                      </div>
                      <div className="rounded-xl bg-amber-50 dark:bg-amber-950/40 p-4 text-center">
                        <p className="text-2xl font-bold text-amber-700 dark:text-amber-300">{permitStatusCounts.pending}</p>
                        <p className="mt-1 text-xs text-amber-500 dark:text-amber-400">Pending</p>
                      </div>
                      <div className="rounded-xl bg-rose-50 dark:bg-rose-950/40 p-4 text-center">
                        <p className="text-2xl font-bold text-rose-700 dark:text-rose-300">{permitStatusCounts.rejected}</p>
                        <p className="mt-1 text-xs text-rose-500 dark:text-rose-400">Rejected</p>
                      </div>
                      <div className="rounded-xl bg-slate-100 dark:bg-slate-700/60 p-4 text-center">
                        <p className="text-2xl font-bold text-slate-800 dark:text-slate-200">{permitStatusCounts.expired}</p>
                        <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">Expired</p>
                      </div>
                    </div>
                  ...
                  <div className="flex flex-wrap items-center gap-2 self-start">
                    {canManageFinancials && (
                      <button
                        ...
                        className="inline-flex items-center gap-2 rounded-lg border border-emerald-200 dark:border-emerald-700 bg-emerald-50 dark:bg-emerald-950/40 px-3 py-1.5 text-xs font-medium text-emerald-700 dark:text-emerald-300 hover:bg-emerald-100 dark:hover:bg-emerald-900/50"
                      >
                        ...
                        {filteredStudents.map((student) => {
                          ...
                          return (
                            <tr key={student.id} className="hover:bg-gray-50">
                              ...
                              <td className="px-5 py-3 font-medium text-green-700 dark:text-green-400">{formatMoney(student.amountPaid, activeCurrencyCode)}</td>
                              <td className="px-5 py-3">
                                <span className={`font-semibold ${student.feesBalance > 0 ? 'text-red-600 dark:text-red-400' : 'text-green-600 dark:text-green-400'}`}>
